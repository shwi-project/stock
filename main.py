...
# ─────────────────────────────────────────────
st.markdown("---")
_tab_scanner, _tab_analysis = st.tabs(["🔎 모닝 스캐너", "📊 종목 분석"])
...
with _tab_scanner:
    ...
    if _scanner_df.empty:
        ...
        for _idx, _row in _scanner_df.iterrows():
            ...
            # 카드 헤더 (항상 표시)
            st.markdown(f'''
            <div class="scanner-card" id="scanner-{_row["code"]}">
            ...
            # AI 브리핑 (expander로 클릭 시 펼침)
            with st.expander(f"💡 {_row['name']} AI 브리핑 보기", expanded=False):
                _code = _row["code"]
    ...
    st.markdown(
        ...
        '💡 종목 분석 탭에서 상세 분석을 확인하세요 · 스캔 데이터는 10분 캐시</div>',
        unsafe_allow_html=True
    )

# ─── 탭 2: 종목 분석 (하단에서 _tab_analysis 컨텍스트 내 렌더) ───
_run_analysis = False
if selected_label is not None:
    ...
    if not _query_btn:
        if st.session_state.get("cached_data_key") is not None:
            # 캐시 있으면 이전 결과 유지하면서 표시
